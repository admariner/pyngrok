...
- New `NgrokTunnel.upstream` attribute, populated from the v3 endpoint response (or synthesized from the v2 `config.addr` for backward compatibility). Other v3 endpoint response fields (e.g. `traffic_policy`, `bindings`) remain accessible via the raw `NgrokTunnel.data` dict.
